...
* Changed - Icon fonts are now loaded only when a filter preset actually uses them. The plugin bundles five families, and previously loaded all of them on every page. Its own tick, close and spinner icons are drawn inline instead, so a page can now load none at all
* Changed - The AJAX response no longer carries a copy of the product query object, which nothing read. Responses are around a third smaller
* Added - swpf_preset_icon_families filter, for themes that render filter markup themselves and need an icon font loaded
