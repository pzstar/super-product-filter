        ...
        if ($terms) {
            foreach ($terms as $key => $term) {
                ...
                if ($tax_name == 'product_visibility') {
                    ...
                    if (isset($current_filter_option['brand']) && !empty($current_filter_option['brand']) && is_array($current_filter_option['brand'])) {
                        $checked = in_array($term->slug, $current_filter_option['brand']) ? true : false;
                    }
                    ?>
                    <div class="swpf-filter-item">
                        <label class="swpf-filter-label">
                            <input type="checkbox" id="swpf-term-<?php echo esc_attr($term->term_id) ?>" class="swpf-chkbox-term swpf-chkbox-term-<?php echo esc_attr($term->term_id); ?>" name="brands[]" data-termurl="<?php echo esc_url(get_term_link($term->term_id)); ?>" data-taxonomy="<?php echo esc_attr($term->taxonomy); ?>" data-termid="<?php echo esc_attr($term->term_id); ?>" value="<?php echo esc_attr($term->slug); ?>" <?php checked($checked, true); ?>>
                            <span class="swpf-title">
                                <span class="swpf-term">
                                    <?php
                                    if (isset($settings['terms_customize'][$tax_name][$term->term_id]['term_name']) && !empty($settings['terms_customize'][$tax_name][$term->term_id]['term_name'])) {
